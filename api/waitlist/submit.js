// Vercel serverless proxy for the waitlist form (pages/waitlist.html).
...
module.exports = async function submitWaitlist(req, res) {
  ...
  if (!process.env.SUPABASE_URL || !process.env.SUPABASE_ANON_KEY) {
    return res.status(503).json({ error: 'The waitlist is being prepared. Please try again shortly.' });
  }
  ...
  try {
    ...
    if (!insertResponse.ok) {
      console.error('Waitlist insert failed', insertResponse.status, await insertResponse.text());
      return res.status(502).json({ error: 'We could not save your waitlist entry. Please try again.' });
    }

    return res.status(200).json({ ok: true });
  } catch (error) {
    console.error('Waitlist submission error', error && error.message);
    return res.status(503).json({ error: 'The waitlist is temporarily unavailable. Please try again.' });
  }
};
