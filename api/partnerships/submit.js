...
module.exports = async function submitPartnershipInquiry(req, res) {
  ...
  const space = String(body.space || '').trim().slice(0, 300);
  ...
  const summaryLines = [
    ...
    space ? `On-site space: ${space}` : null,
    preferredDay ? `Preferred day/time: ${preferredDay}` : null,
  ...
  try {
    ...
    console.error('Partnership inquiry submission error', error && error.message);
