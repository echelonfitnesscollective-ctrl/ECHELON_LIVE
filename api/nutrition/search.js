...
async function authenticated(request) {
  ...
  const response = await fetch(`${process.env.SUPABASE_URL}/auth/v1/user`, { headers: { apikey: process.env.SUPABASE_ANON_KEY, authorization: header } });
  return response.ok;
}
...
export default async function handler(request, response) {
  ...
  const fingerprint = `${request.headers['x-forwarded-for'] || request.socket?.remoteAddress || 'unknown'}:${query.toLowerCase()}`;
