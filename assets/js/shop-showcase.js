...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const slides = collections.map((item, index) => `<article class="collection-slide${index === 0 ? ' active' : ''}" data-collection-slide aria-hidden="${index === 0 ? 'false' : 'true'}"><div class="collection-visual"><img src="${item.image}" alt="${item.alt}"><span>DROP 01 / ${item.number}</span></div><div class="collection-copy"><span class="checkin-tag">${item.label}</span><h3>${item.title}</h3><p>${item.copy}</p><div class="goods-detail-row"><span>${item.details}</span><span>AVAILABLE VIA ETSY</span></div></div></article>`).join('');
