...
function createApplicationRecord(item, questionLabelMap) {
    ...
    function buildCreateOfferForm(reopenLabel) {
        ...
        [['echelon_12_monthly', 'ECHELON 12 · $149 / MO'], ['echelon_12_paid_in_full', 'ECHELON 12 · $399 PAID IN FULL'], ['one_on_one_monthly', '1-ON-1 COACHING · MONTHLY (UP TO 3X/WEEK)'], ['one_on_one_starter', '1-ON-1 COACHING · $55 STARTER SESSION'], ['one_on_one_20pack', '1-ON-1 COACHING · $840 20-SESSION PACK'], ['private_group_training', 'PRIVATE GROUP TRAINING · BY SIZE']].forEach(([value, label]) => { const option = document.createElement('option'); option.value = value; option.textContent = label; choice.append(option); });
